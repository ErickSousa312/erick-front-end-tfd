...
const ButtonCellRenderer = ({ value }) => {
  return <button onClick={() => console.log('oiiii')}>editar</button>;
};

export default function TablePacientes({ Pacientes }) {
  ...
  const columns: GridColDef[] = [
    ...
    {
      field: 'NumeroTituloEleitor',
      headerName: 'Número Título Eleitor',
      width: 200,
    },
    ...
    {
      field: 'NomePaiouResponsavel',
      headerName: 'Nome do Pai ou Responsável',
      width: 200,
    },
  ...
  return (
    <>
      <section>
        ...
        <button
          className={` ${isVariavelVazia() ? 'button2' : 'button'}`}
          onClick={Nextpage}
          disabled={isVariavelVazia()}
        >
          Novo Parecer
        </button>
      </section>
